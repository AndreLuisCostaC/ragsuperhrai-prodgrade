...
resource "aws_lambda_layer_version" "layer1" {
  description         = "Lambda layer 1"
  filename            = "${path.module}/../dist/layer1.zip"
  layer_name          = "layer1"
  compatible_runtimes = ["python3.12"]
  source_code_hash    = filebase64sha256("${path.module}/../dist/layer1.zip")
}

resource "aws_lambda_layer_version" "layer2" {
  description         = "Lambda layer 2"
  filename            = "${path.module}/../dist/layer2.zip"
  layer_name          = "layer2"
  compatible_runtimes = ["python3.12"]
  source_code_hash    = filebase64sha256("${path.module}/../dist/layer2.zip")
}

resource "aws_lambda_layer_version" "layer3" {
  description         = "Lambda layer 3"
  filename            = "${path.module}/../dist/layer3.zip"
  layer_name          = "layer3"
  compatible_runtimes = ["python3.12"]
  source_code_hash    = filebase64sha256("${path.module}/../dist/layer3.zip")
}
# &&&

# Lambda function
#S3_BUCKET              = aws_s3_bucket.memory.id
#S3_BUCKET_NAME          = aws_s3_bucket.snowflake-s3-bucket-to-snowflake.id
...
resource "aws_lambda_function" "api" {
  ...
  tags             = local.common_tags
  layers           = [aws_lambda_layer_version.layer1.arn, aws_lambda_layer_version.layer2.arn, aws_lambda_layer_version.layer3.arn]

  environment {
    variables = {
      CORS_ORIGINS           = var.use_custom_domain ? "https://${var.root_domain},https://www.${var.root_domain}" : "https://${aws_cloudfront_distribution.main.domain_name}"
      S3_BUCKET_NAME         = aws_s3_bucket.memory.id
      USE_S3                 = "true"
